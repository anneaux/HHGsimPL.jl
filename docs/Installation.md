To use ATIsimPL.jl and/or HHGsimPL.jl you'll first need to install the programming language julia and make it usable in jupyter notebooks (aka installing a julia kernel). Then, you can download/clone the relevant code & notebooks I prepared to start running your own calculations.
...
- make sure you have jupyter notebook installed somehow
...
- get the code from the github repo I sent you: Select the `user` branch in the drop-down menu (where it initially says `master`) and get the code by either cloning the repo (if you know git) or simply download the code as a zip and unpack it.
...
```
add StaticArrays, NLsolve, Contour, QuadGK, Sobol, LinearAlgebra, Integrals, FiniteDiff, Peaks
```
- open the notebook called "get HHG dipoles" and see if it works.
