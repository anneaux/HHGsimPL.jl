# adapted & modified for my purposes from here https://github.com/bdeonovic/Clustering.jl/blob/91e12df4b44827da3515c8cc8e663eac300bc52a/src/meanshift.jl

module MeanShiftClustering

export MeanShiftResult, meanshift, modedetect

using Distances
struct MeanShiftResult{T <: AbstractFloat} 
...
#1-d profile
function g1(xi::Vector{T}, x::T, h::Float64) where T <: AbstractFloat
  (1/2)*exp.(-1/2*((x .-xi)/h).^2)
end

#multi-d profile
function gd(Xi::Matrix{T},x::Vector{T},h::Vector{Float64}) where T <: AbstractFloat
  d = size(x,1)
  k = ones(T,size(Xi,1))
  for j in 1:d
...
function ms(X::Matrix{T}, x::Vector{T}, h::Vector{Float64}) where T <: AbstractFloat
  d = size(X,2)
  g = gd(X,x,h)
...
function ms(X::Matrix{T}, x::Vector{T}, h::Float64) where T <: Real
  d = size(x,1)
  ms(X,x,Float64[h for j in 1:d])
end

function msrep(X::Matrix{T}, x::Vector{T}, h::Vector{Float64}; threshold::Float64 = eps(), iter::Int64 = 200) where T <: AbstractFloat
  s = 0
...
function msrep(X::Matrix{T}, x::Vector{T}, h::Float64; threshold::Float64 = eps(), iter::Int64 = 200) where T <: AbstractFloat
  d = size(X,2)
...
function meanshift(
  ...
  scaled::Bool = true,
  iter::Int64=200) where T <: AbstractFloat
  ...
  s1 = ones(T,d)
  if scaled
    s1 =  mapslices( (x) -> Distances.spannorm_dist(x,zeros(size(x,1))), X ,1)[:]
  ...
  counts = Array{Int64}(undef, 0)#Array(Int64,0)
...
function meanshift(
  ...
  s1 =  mapslices( (x) -> Distances.spannorm_dist(x,zeros(n)), X ,dims=1)[:]
...
function meanshift(x::Vector{T}; maxsamples::Int64=1000) where T <: AbstractFloat
  ...
  meanshift(hcat(x,x),subset=subset,scaled=false)
